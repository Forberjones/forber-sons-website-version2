...
export default function GalleryClient({
  ...
  function to(i: number) {
    ...
    if (el) {
      // block: "nearest" prevents the page jumping vertically
  ...
  function next() {
    to((index + 1) % images.length);
  }

  function prev() {
    to((index - 1 + images.length) % images.length);
  }
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const offsets = child.map((c) =>
        Math.abs(c.getBoundingClientRect().left - el.getBoundingClientRect().left)
      );
  ...
  return (
    <div className="relative">
      ...
      >
        {images.map((src, i) => (
          <div key={i} className="relative snap-start shrink-0 w-full aspect-[16/11]">
            <Image
              src={src}
              alt={`${title} image ${i + 1}`}
              fill
